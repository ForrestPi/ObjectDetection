...
class SENet(nn.Module):
    ...
    def forward(self, x, out_inds):
        x = self.features(x, out_inds)
        return x
...
def initialize_pretrained_model(model, num_classes, settings):
    ...
    model_dict = {}
    for k, v in list(pretrain_dict.items()):
        if not k.startswith('last_linear'):
            model_dict[k] = v
    import torch
    model_dict.update(
        {'last_linear.weight': torch.rand(num_classes, 2048), 'last_linear.bias': torch.zeros(num_classes)})
