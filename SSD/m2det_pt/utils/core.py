...
from data import COCODetection, VOCDetection, detection_collate, preproc, CustomHelmetDataset
...
def adjust_learning_rate(optimizer, gamma, epoch, step_index, iteration, epoch_size, cfg):
    global lr
    if epoch <= 5:
        lr = cfg.train_cfg.end_lr + \
             (cfg.train_cfg.lr[0] - cfg.train_cfg.end_lr) * iteration / (epoch_size * cfg.train_cfg.warmup)
    else:
...
# I change adjust learning rate strategy
def adjust_learning_rate_v2(optimizer, epoch, cfg):
    global lr
    # if epoch < 5,
    if epoch <= 5:
        lr = (cfg.train_cfg.lr[0] - cfg.train_cfg.end_lr) * (epoch+1) / 6
    else:
        for i in range(len(cfg.train_cfg.step_lr.COCO)):
            if epoch <= cfg.train_cfg.step_lr.COCO[i]:
                lr = cfg.train_cfg.lr[i]
                break
        # lr = cfg.train_cfg.init_lr * (gamma ** (step_index))
    for param_group in optimizer.param_groups:
...
def adjust_learning_rate_helmet(optimizer, epoch, cfg):
    global lr
    # if epoch < 5,
    if epoch <= 5:
        lr = (cfg.train_cfg.lr[0] - cfg.train_cfg.end_lr) * (epoch+1) / 6
    else:
        for i in range(len(cfg.train_cfg.step_lr.HELMET)):
            if epoch <= cfg.train_cfg.step_lr.HELMET[i]:
                lr = cfg.train_cfg.lr[i]
                break
        # lr = cfg.train_cfg.init_lr * (gamma ** (step_index))
    for param_group in optimizer.param_groups:
...
def print_train_log(iteration, print_epochs, info_list):
    if iteration % print_epochs == 0:
        cprint(
            'Time:{}||Epoch:{}||EpochIter:{}/{}||Iter:{}||Loss_L:{:.4f}||Loss_C:{:.4f}||Batch_Time:{:.4f}||LR:{:.7f}'.format(
                *info_list), 'green')
...
def save_checkpoint(net, cfg, final=True, datasetname='COCO', epoch=10):
    if final:
        torch.save(net.state_dict(), cfg.model.weights_save + \
                   'Final_M2Det_{}_size{}_net{}.pth'.format(datasetname, cfg.model.input_size,
                                                            cfg.model.m2det_config.backbone))
    else:
        torch.save(net.state_dict(), cfg.model.weights_save + \
                   'M2Det_{}_size{}_net{}_epoch{}.pth'.format(datasetname, cfg.model.input_size,
                                                              cfg.model.m2det_config.backbone, epoch))


def write_logger(info_dict, logger, iteration, status):
    if status:
        for tag, value in list(info_dict.items()):
