...
arch = 'dla_34'  # res_18, res_101, hourglass
heads = {'hm': 80, 'reg': 2, 'wh': 2}
head_conv = 256  # 64 for resnets
model_path = './weights/ctdet_coco_dla_2x.pth'
...
if __name__ == "__main__":
    print('Export DLA not supported now due to DCN lack in ONNX')
    detector = CenterNetDetector()
    data_f = 'images/33887522274_eebd074106_k.jpg'
    if len(sys.argv) > 1:
        data_f = sys.argv[1]
    detector.export_onnx(data_f, 'centernet_dla.onnx')
    # detector.run(data_f)
    print('onnx exported!')