...
class Debugger(object):
    def __init__(self, ipynb=False, theme='black',
        ...
        self.colors = np.array(colors, dtype=np.uint8).reshape(
            len(colors), 1, 1, 3)
        self.colors_3d = [(255, 0, 255), (255, 255, 0), (255, 0, 255), (0, 255, 255)]
        if self.theme == 'white':
            self.colors = self.colors.reshape(-1)[::-
                                                  1].reshape(len(colors), 1, 1, 3)
            self.colors = np.clip(self.colors, 0., 0.7 * 255).astype(np.uint8)
        self.dim_scale = 1
        if dataset == 'coco_hp':
            self.names = ['p']
            self.num_class = 1
  ...
  '''
    ...
    def show_result_only(self, pause=False):
        res = self.imgs['add_pred']
        cv2.imshow('result', res)
        if cv2.waitKey(0 if pause else 1) == 27:
            import sys
            sys.exit(0)

    def save_img(self, imgId='default', path='./images/debug/'):
        os.makedirs(path, exist_ok=True)
        cv2.imwrite(path + '{}.png'.format(imgId), self.imgs[imgId])

    def save_all_imgs(self, path='./images/debug/', prefix='', genID=False):
        os.makedirs(path, exist_ok=True)
        if genID:
            try:
